...
export function BookDetailScreen({ navigation, route }: Props) {
  ...
  useEffect(() => {
    void refreshBookDetail().catch(() => {
      setLoadError(t.bookDetail.loadFailed);
    });
  }, [refreshBookDetail, t.bookDetail.loadFailed]);
  ...
  async function joinSeries(series: Series) {
    try {
      ...
      setLoadError(t.bookDetail.joinFailed);
    }
  }
  ...
  if (!book) {
    return (
      <SafeAreaView style={[styles.root, { backgroundColor: theme.background }]}>
        <Text style={[styles.muted, { color: theme.muted }]}>{t.bookDetail.missing}</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView edges={["bottom"]} style={[styles.root, { backgroundColor: theme.background }]}>
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.heroText}>
          ...
          <Text style={[styles.author, { color: theme.muted }]}>{book.author ?? t.common.unknownAuthor}</Text>
        </View>
      </View>

      <View style={styles.actions}>
        <PrimaryButton onPress={() => navigation.navigate("Reader", { bookId: book.id })}>{t.bookDetail.startReading}</PrimaryButton>
        <PrimaryButton variant="secondary" onPress={() => setSeriesPickerVisible(true)}>
          {t.bookDetail.joinSeries}
        </PrimaryButton>
      </View>

      <View style={[styles.seriesSummary, { backgroundColor: theme.panel, borderColor: theme.border }]}>
        <View style={styles.seriesSummaryText}>
          <Text style={[styles.sectionTitle, { color: theme.text }]}>{t.bookDetail.series}</Text>
          <Text style={[styles.muted, { color: theme.muted }]}>{bookSeries[0]?.name ?? t.bookDetail.noSeries}</Text>
        </View>
      </View>
        ...
        <View style={styles.modalScrim}>
          <View style={[styles.pickerPanel, { backgroundColor: theme.background }]}>
            <View style={styles.pickerHeader}>
              <View style={styles.seriesSummaryText}>
                <Text style={[styles.pickerTitle, { color: theme.text }]}>{t.bookDetail.joinSeries}</Text>
                <Text style={[styles.muted, { color: theme.muted }]}>{t.bookDetail.singleSeriesOnly}</Text>
              </View>
            ...
            <FlatList
              ...
              ListEmptyComponent={<Text style={[styles.muted, { color: theme.muted }]}>{t.bookDetail.noSeriesCreated}</Text>}
              renderItem={({ item }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={styles.seriesState}>{selected ? t.bookDetail.current : t.bookDetail.join}</Text>
                  </Pressable>
                );
              }}
            />
